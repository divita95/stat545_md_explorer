...
```{r}
x <- 1:10
map(x, sqrt) #always ouput is a list
```
...
got_chars %>% {
    tibble(
        name = map_chr(., "name"),
        born = map_chr(., "born")
    ) # the dot represents include whatever is on the left hand side of the pipe operator. the curly braces are imp or else it will give a tibble.
